...
public class OrderKafkaConsumer {
    ...
    public void consumeOrder(ConsumerRecord<String, OrderEvent> orderRecord) {
        ...
        try {
            ...
            processingExecutor.submit(() -> processOrderEvents(orderRecord.key(), orderRecord.topic(), orderId));
    ...
    private boolean offerEventToQueue(String orderId, OrderEvent event) {
    ...
    private void processOrderEvents(String productId, String topicName, String orderId) {
        try {
            ...
            while ((event = queue.poll()) != null) {
                processSingleEventWithRetry(productId, topicName, event);
            }
        } finally {
    ...
    private void processSingleEventWithRetry(String productId, String topicName, OrderEvent event) {
        var attempt = 0;
        while (attempt < 3) {
            try {
                processSingleEvent(productId, topicName, event);
                return;
            } catch (Exception e) {
                attempt++;
                if (attempt >= 3) {
    ...
    private void processSingleEvent(String productId, String topicName, OrderEvent event) {
        try {
            ...
            switch (event.getEventType()) {
                case ORDER_CREATED -> orderService.create(productId, topicName, order);
                case ORDER_UPDATED -> {
